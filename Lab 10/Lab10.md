...
2.  Open the **cloud shell** by clicking on the cloud shell icon on the Azure portal.
...
11. Back in the Cloud Shell command prompt, submit the job to train the
    ...
    ```
    ::: secondary
    **Note:** If this script does not get pasted properly, use the contents of the file **Scriptfile** in **C:\Labfiles**
    :::
...
3.  Click on **Save as a new cohort**. The **Filter** in this dataset
...
4.  From reviewing the chart, you will see that the model performance
    for all test data cohort and erroneous cohort is correct at ~70% of
    the time. 

5.  The **Precision score** rate for the **least erroneous cohort** is
