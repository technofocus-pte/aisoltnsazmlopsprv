...
5.	Open the Notebook(the .ipynb file), **automl-regression-task-hardware-performance**. The Notebook is automatically connected to the **compute** that we created earlier. Ensure that the compute is in the **Running** state.
...
8.	Execute the next cell to get the **Execution Summary**.
...
3.	Check for the notification stating **Endpoint "regression-<Currentdate&time>" update completed** or check the status under **Endpoints**.
